...
- **Keep the native MXFP4 weights — don't re-quantize.** gpt-oss-20b is
  ...
  levers on a ≤16 GB GPU are the KV cache and context length, below.
- **`-c 65536`** = context window. A code agent burns context fast (tool defs +
  reading files + reasoning); 64k is a comfortable target, 32k the practical
  minimum. gpt-oss supports up to `-c 131072` if you have the memory (add
  `--n-cpu-moe 4` to offload some experts to CPU RAM and make room).
- **`-fa on -ctk q8_0 -ctv q8_0`** = flash attention + **8-bit KV cache**. This
  is the single biggest memory lever — it ~halves KV-cache size with negligible
  quality loss, which is what makes long agent sessions fit. (Avoid 4-bit KV:
  it noticeably degrades long-context recall and tool-argument accuracy.)

Sampling is set by the agent, not the server: it sends `temperature` **and**
`top_p` together (see `AGENT_TEMPERATURE` / `AGENT_TOP_P` in the env-var table).
gpt-oss's own recommendation is `temperature=1.0, top_p=1.0`; the agent defaults
a bit lower for steadier tool-call formatting, with the malformed-header salvage
as a backstop. If tool calls look unreliable, A/B `AGENT_TEMPERATURE=1.0` on the
box — that trade-off can only be settled by a live run.
...
## Step 3 — (recommended) build the project map — `local init`
...
```bash
python cli.py --project /path/to/repo init     # or:  --init
```
...
In the TUI, run `/init` any time to (re)build it. A fingerprint of the source tree is
saved alongside it (`.local_mind.sig.json`, git-ignored); when the code changes
**massively** (many files added/removed/resized, or a git HEAD move with edits), the TUI
flags `local_mind.md` as stale at startup and suggests `/init`. Set
`AGENT_MIND_AUTO_REFRESH=1` to have it regenerate automatically instead.
...
**`/`-command autocomplete**. Slash commands: `/help`, `/init`, `/reasoning low|medium|high`,
`/show-reasoning`, `/exec on|off`, `/mode plan|ask|accept`, `/model`, `/tokens`,
`/shortcuts`, `/clear`, `/exit`. **`Ctrl-C` interrupts a
...
## Step 5 — full-loop test (recommended)
...
| `AGENT_TEMPERATURE` | `0.6` | sampling temperature; gpt-oss recommends `1.0`, agent defaults lower for steadier tool calls (salvage backstops it). A/B on the box |
| `AGENT_TOP_P` | `1.0` | nucleus sampling — sent alongside temperature (gpt-oss recommends `1.0`) |
